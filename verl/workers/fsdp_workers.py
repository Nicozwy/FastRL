...
class FSDPWorker(Worker):
    ...
    def _process_multi_modal_inputs(self, data: DataProto):
        ...
        current_batch_size = len(current_uids)

        # 1. 先检查缓存的 uid 长度是否和当前 batch 一致，不一致直接清缓存
        if "uid" in self._cache:
            cached_uids = self._cache["uid"]
            # 长度不一致 → 清缓存（pruning 后 batch 被裁剪，长度变了）
            if len(cached_uids) != current_batch_size:
                # print("长度不一致！！！")
                self._cache.clear()
            # 长度一致但 uid 不匹配 → 清缓存（确保 current_uids 是 numpy 数组再比较）
            elif not np.all(np.asarray(current_uids) == np.asarray(cached_uids)):
                # print("uid 不匹配！！！")
                self._cache.clear()

        # 2. 缓存不存在或已被清空 → 重新处理多模态数据并缓存
        if "multi_modal_inputs" not in self._cache:
            min_pixels = data.meta_info["min_pixels"]
            max_pixels = data.meta_info["max_pixels"]
            video_fps = data.meta_info["video_fps"]
            batch_multi_modal_inputs = []
            multi_modal_inputs_cache = {}  # 避免重复处理相同 uid 的数据

            for index, multi_modal_data in zip(current_uids, data.non_tensor_batch["multi_modal_data"]):
                if index not in multi_modal_inputs_cache:
                    images, videos = [], []
                    if "images" in multi_modal_data:
                        for image in multi_modal_data["images"]:
            ...
            # 关键：转换为 numpy 对象数组（支持 list 索引）
            self._cache["uid"] = current_uids
            self._cache["multi_modal_inputs"] = np.array(batch_multi_modal_inputs, dtype=object)
        else:
            # 3. 缓存存在且 uid 匹配 → 直接复用缓存（numpy 数组）
            batch_multi_modal_inputs = self._cache["multi_modal_inputs"]

        # 关键：赋值时确保是 numpy 对象数组（支持 list 索引）

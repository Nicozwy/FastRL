...
def prepare_dynamic_batch(data: DataProto, max_token_len: int) -> tuple[list[DataProto], list[list[int]]]:
    ...
    for i, batch_idx in enumerate(batch_idx_list):
        ...
        # non_tensors = {key: value[batch_idx] for key, value in data.non_tensor_batch.items()} #原来
        # edit
        non_tensors = {}
        for key, value in data.non_tensor_batch.items():
            # 把 value 转成 numpy 对象数组，确保支持 list 索引
            value_np = np.asarray(value, dtype=object)
            non_tensors[key] = value_np[batch_idx]

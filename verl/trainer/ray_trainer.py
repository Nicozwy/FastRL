...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        print(f"run_time is {run_time}")
        self.acc_file_path = f"logs/geo3k/train_question_acc_{run_time}.jsonl"
        # self.acc_file_path = f"logs/geoqa8k/train_question_acc_{run_time}.jsonl"
        self.acc_map = {}  # 缓存准确率映射 {question_id: (acc_list, overall_list)}
        self.filter_refresh_interval = 1  # 每1步刷新一次过滤列表
        self.skip_next_filter: Set[int] = set()

        # 过滤参数（仅保留beta和r）
        self.filter_beta = 0.5
        self.max_filter_ratio = 0.2
        self.ema_map: Dict[int, float] = {}
        self.ema_seen_len: Dict[int, int] = {}
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                if baseline_cppo:
                    ...
                    # 保存日志
                    with open(f"logs/geo3k/pruning_cppo_grpo_{run_time}.txt", "a") as f:
                        for line in txt_lines:
                            f.write(line + "\n")

                    print(f"Pruning 日志已保存到 logs/geo3k/pruning_cppo_grpo_{run_time}.txt")
                else:
                    print("不启动CPPO！")
